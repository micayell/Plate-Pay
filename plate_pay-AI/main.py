...
@app.post("/api/v1/ocr/license-plate")
async def ocr_license_plate(file: UploadFile = File(...)):
    ...
    try:
        ...
        if unique_plates:
            return JSONResponse({
                "success": True,
                "plate_number": unique_plates[0]["text"]
            })
        else:
            return JSONResponse({
                "success": False,
                "message": "번호판을 찾지 못했습니다."
            })
        
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"이미지 처리 중 오류 발생: {str(e)}")

@app.post("/api/v1/ocr/enter-ocr")
async def ocr_license_plate_direct(file: UploadFile = File(...)):
    """YOLO 없이 번호판 이미지에서 직접 텍스트를 추출하는 엔드포인트"""
    ...
    try:
        ...
        # 원본 이미지에서 OCR 수행
        original_results = reader.readtext(image_array)
        
        # 이미지 향상 후 OCR 수행
        enhanced_image = enhance_license_plate(image_array)
        enhanced_results = reader.readtext(enhanced_image)
        
        # 기본 전처리 후 OCR 수행 (추가 옵션)
        preprocessed_image = preprocess_image(image_array)
        preprocessed_results = reader.readtext(preprocessed_image)
        
        # 모든 결과 합치기
        combined_results = original_results + enhanced_results + preprocessed_results
        
        # 번호판 텍스트 추출
        license_plates = extract_license_plate_text(combined_results)
